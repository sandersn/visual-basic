...
Select Case tabCDLib.Tab
    Case 0  'general
        ...
        cmdSave.Default = True  'set the default so the user can easily add records
...
Select Case tabCDLib.Tab
    Case 0
        ...
        cmdSave.Default = True  'so the user can just press Enter(using the keyboard)
...
Select Case tabCDLib.Tab
    Case 0
        ...
        cmdAdd.Default = True 'make this one the default so that they can easily add records
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
     If KeyCode = vbKeyLeft Then    'preview the keys so that we can 'click' the data
        'controls
        Select Case tabCDLib.Tab
        'general not working yet because of Reposition event. How do I get around this?
'            Case 0  'general
'                datMusicHdr.Recordset.MovePrevious
'                If datMusicHdr.Recordset.BOF Then datMusicHdr.Recordset.MoveFirst
            Case 1  'notes
            Case 2  'categories
                datCatChange.Recordset.MovePrevious
                If datCatChange.Recordset.BOF Then datCatChange.Recordset.MoveFirst
        End Select
     ElseIf KeyCode = vbKeyRight Then
        Select Case tabCDLib.Tab
'               Case 0  'general
'                   datMusicHdr.Recordset.MoveNext
'                   If datMusicHdr.Recordset.EOF Then datMusicHdr.Recordset.MoveLast
               Case 1  'notes
               Case 2  'categories
                    datCatChange.Recordset.MoveNext
                    If datCatChange.Recordset.EOF Then datCatChange.Recordset.MoveLast
           End Select
     End If
End Sub
...
OldIndex = 3    'start off at Default
    Select Case Index
        Case 0
